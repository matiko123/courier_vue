            ...
            <div class="page-header">
              <nav class="breadcrumb-one" aria-label="breadcrumb">
                <ol class="breadcrumb pt-2 ml-3">
                  ...
                  <li class="breadcrumb-item active" aria-current="page"><span>Collection-Points</span></li>
                  ...
                  <div class="row align-items-center justify-content-center">
                    ...
                    <div class="col-auto">
                      <div class="input-group mb-2">
                        <input type="date" class="form-control" id="endDate" v-model="endDate" placeholder="End Date" required />
                      </div>
                    </div>
                    <div class="col-auto">
                      <div class="input-group mb-2">
                        <multiselect v-model="selectedBranch" :options="branches" :searchable="true" placeholder="Select Branch" :custom-label="customLabel" required></multiselect>
                      </div>
                    </div>
...
export default {
  ...
  methods: {
    ...
    fetchBranches() {
      this.axiosInstance
        .get('masters/collection-branches')
        .then((response) => {
          const branchesArray = Object.entries(response.data.data).map(([id, name]) => {
            return { id, name };
          });
    ...
    async printData() {
      ...
      if (!this.selectedBranch || !this.selectedBranch.id) {
        this.$refs.toastNotification.showErrorToast(`Error Data not found for selected Branch.`);
        return;
      }
      ...
      try {
        ...
        const url = `${this.axiosInstance.defaults.baseURL}collection/route/generate?${params}`;
        if (url) {
          window.open(url, '_blank');
          this.$refs.toastNotification.showSuccessToast('Printing Collection points report.');
        }
      } catch (error) {
        console.error('Error printing Collection points report:', error);
        this.$refs.toastNotification.showErrorToast(`Error printing Collection points report: ${error.message}`);
      } finally {
