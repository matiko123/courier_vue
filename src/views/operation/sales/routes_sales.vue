                ...
                <div class="invoice-inbox p-4">
                  <form class="d-flex justify-content-center">
                    <div class="row">
                      ...
                      <div class="col-xl-2 col-md-3 col-sm-6 d-none">
                        <div class="col-auto">
                      ...
                      <div class="col-xl-2 col-md-3 col-sm-6 d-none">
                        <div class="col-auto">
                          <div class="input-group mb-2">
                            <multiselect v-model="selectedBranch" :options="branches" :searchable="true" placeholder="Select Branch" label="label"></multiselect>
                          </div>
                        </div>
                      </div>
                      <div class="col-md-4">
                        <div class="buttons p-2">
                          <button class="btn btn-danger d-none" @click.prevent="resetSelection">Reset</button>
          ...
          <div class="invoice-inbox p-4">
            <v-client-table :data="items" :columns="columns">
              ...
              <template #sender="props">
                <div class="d-flex">
                  <p class="align-self-center centered mb-0 admin-name">{{ props.row.sender.name }}</p>
                </div>
              </template>
              <template #parcel_type="props">
                <div class="d-flex">
                  <p class="align-self-center centered mb-0 admin-name">{{ props.row.parcels[0].parcel_type }}</p>
                </div>
              </template>
              <template #receiver="props">
                <div class="d-flex">
                  <p class="align-self-center centered mb-0 admin-name">{{ props.row.parcels[0].receiver_detail.name }}</p>
                </div>
              </template>
              <template #total_amount="props">
                <div class="d-flex">
                  <p class="align-self-center centered mb-0 admin-name">{{ props.row.parcels[0].total_amount }}</p>
                </div>
              </template>
  ...
  methods: {
    ...
    async fetchData() {
      try {
        ...
        const response = await axiosInstance.get('reports/sales-by-agents', {
          params,
        });
    ...
    async printData() {
      try {
        ...
        const url = `${axiosInstance.defaults.baseURL}reports/sales-by-agents?${params}`;
        if (url) {
          window.open(url, '_blank');
          this.$refs.toastNotification.showSuccessToast('Printing route sales report.');
        }
      } catch (error) {
        console.error('Error printing route sales report:', error);
        this.$refs.toastNotification.showErrorToast(`Error printing route report: ${error.message}`);
      } finally {
