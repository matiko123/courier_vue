          ...
          <div class="modal-body">
            <form @submit.prevent="handleSubmit">
              <div class="row">
                ...
                <div class="col-md-6 mb-3">
                  <label for="company" class="form-label">Entity</label>
                  <multiselect v-model="selectedEntity" :options="entity" :searchable="true" placeholder="Select Entity" :custom-label="customLabel" required />
                </div>
              </div>
...
export default {
  ...
  methods: {
    ...
    async createData() {
      try {
        const response = await axios.get('https://authgateway.abooderp.com/api/v1/company-entity/create');
        const data = response.data.data
        this.company = data.company_detail
        this.entity = data.entity_detail
      } catch (error) {
    ...
    async handleSubmit() {
      if (!this.selectedCompany || !this.selectedEntity) {
        this.$refs.toastNotification.showInfoToast('Please fill in all fields.');
        return;
      }

      const postData = new FormData();
      postData.append('company_id', this.selectedCompany.id);
      postData.append('entity_id', this.selectedEntity.id);
 
      try {
        this.register = true;
        const response = await axios.post('https://courierapi.iosuite.org/api/v1/company-entity/store', postData, {
          headers: {
    ...
    async fetchData() {
      try {
        const response = await axios.get('https://authgateway.abooderp.com/api/v1/company-entity/index');
  ...
  computed: {
    columns() {
      return ['sn', 'company_name', 'entity_name'];
    },
  },
};
</script> 
